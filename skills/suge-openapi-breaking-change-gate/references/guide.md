...
`markdown_summary` 是给人看的，因此其中所有来自输入的字符串（规范 `info.version`、`paths` 键、change `detail`、调用方 `client_id` 与 reasons、迁移清单条目）都按**不可信文本**渲染：
...
- 只有 `markdown_summary` 被改写；`changes[]`、`change_id`、`status_counts`、`consumer_impact[]`、`waivers` 等 JSON 结构化值**原样保留**，分级与状态判定完全不变。
- `injection_flagged[]` 是唯一证据来源，每条为 `{"location", "rule", "chars", "sha256_prefix"}`：
  - `location` 是渲染位置（如 `markdown.changes[2].col4`、`markdown.new_version`），用行号与字段名定位，不复制原文；
...
- 该渲染不影响远程 `$ref` 不取回、`PARTIAL` 降级与离线只读边界。
