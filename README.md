...
> 48 个面向中国用户的实用 AI 技能合集 —— 每个都能独立安装、独立使用，覆盖内容创作、研究、电商、法律、办公自动化、智能家居、金融投资等高频场景。
...
| 28 | 渠道回款拆分对账 | `suge-marketplace-payout-reconcile` | 流水合计 vs 平台声明 vs 银行实收三层核对，银行未到账不判欠款 |
| 29 | 对象存储生命周期变更预演 | `suge-object-storage-lifecycle-preview` | 转换/到期规则映射批次做命中/费用/删除语义预演，不调用云 API |
| 30 | 备份 RPO 恢复点覆盖审计 | `suge-backup-rpo-coverage-audit` | 成功未过期恢复点对照目标 RPO/保留期，恢复测试单独评价 |
...
| 40 | SaaS 离职账号与访问撤销覆盖审计 | `suge-saas-offboarding-access-audit` | 按 subject→system→account→action 建覆盖图，禁登录≠会话/令牌失效，删账号前先交接 |
| 41 | 数据迁移行数与结构一致性门禁 | `suge-data-migration-reconcile-gate` | 先校验快照与增量窗口可比性，再查结构漂移/行数差/内容差，哈希口径不一致拒绝比较 |
| 42 | LLM 评测基线回归门禁 | `suge-llm-eval-regression-gate` | 样本级变化 + 关键用例失败 + slice 覆盖，裁判/量表/数据集漂移判不可比，聚合分不掩盖关键回归 |
| 43 | 门店交接班准备包 | `suge-retail-shift-handover-pack` | 把口头交代整理成可核对交接板，分组未完成事项、现金差异、设备停机与待确认缺口 |
| 44 | 现场服务每日进度更新包 | `suge-field-service-daily-update-pack` | 同源生成内部日报与客户进度稿，无记录任务不冒充完成，照片只做文件名索引 |
| 45 | 退货反馈到详情页修正地图 | `suge-return-feedback-listing-fix-map` | 将脱敏退货反馈聚类到可改字段，样本不足只收集证据，不进入改版任务单 |
| 46 | 电商活动上线准备包 | `suge-ecommerce-campaign-launch-pack` | 把电商活动上线资料整理成可复核上线作战单：逐渠道准备矩阵、价格库存优惠冲突、素材与负责人缺口、发送前确认单 |
| 47 | 小团队 SOP 自动化机会地图 | `suge-small-team-sop-automation-map` | 逐步骤判定保持人工/辅助/可自动化候选/证据不足，给出优先级矩阵、人工检查点、失败回退与实施顺序 |
| 48 | 供应商样品评估准备包 | `suge-supplier-sample-evaluation-pack` | 把样品与规格整理成逐规格证据矩阵，未测、缺证据、单位不可比均不判通过，并给复测与追问清单 |
